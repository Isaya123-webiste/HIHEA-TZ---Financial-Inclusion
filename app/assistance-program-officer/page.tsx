...
export default function AssistanceProgramOfficerDashboard() {
  ...
  useEffect(() => {
    async function checkRoleAndLoadData() {
      try {
        ...
        if (profile.role !== "assistance_program_officer") {
          setError(`Access denied. Your role is '${profile.role}', but assistance_program_officer role is required.`)
          setLoading(false)
          return
        }
  ...
  return (
    <RoleLayout userRole="assistance_program_officer" userName={userProfile?.full_name}>
