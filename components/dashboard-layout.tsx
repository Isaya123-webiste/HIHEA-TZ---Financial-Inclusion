    ...
    <div className="flex h-screen bg-gray-50">
      ...
      >
        ...
        <div className="flex h-16 items-center justify-between border-b px-4">
          {!isCollapsed && (
            <div className="flex items-center gap-2">
              <div className={`rounded-lg bg-${accentColor}-600 p-2`}>
                <span className="material-icons text-white text-lg">admin_panel_settings</span>
              </div>
              <span className="font-bold text-gray-900">HIH {title}</span>
            </div>
          )}
          {isCollapsed && (
        ...
        <nav className="flex-1 space-y-1 p-2">
          {navigationItems.map((item) => {
            const isActive =
              pathname === item.href ||
              (item.href !== `/${userRole.toLowerCase().replace("_", "-")}` && pathname.startsWith(item.href))
            return (
              <Link
                key={item.name}
                href={item.href}
                className={`
                  ...
                  ${isActive ? `bg-${accentColor}-100 text-${accentColor}-700` : "text-gray-700 hover:bg-gray-100 hover:text-gray-900"}
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <div className="flex h-16 items-center justify-between border-b bg-white px-4 lg:hidden">
          ...
          <h1 className="font-bold text-gray-900">HIH {title}</h1>
