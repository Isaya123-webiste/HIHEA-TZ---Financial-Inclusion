...
export default function BranchReportOfficerDashboard() {
  ...
  useEffect(() => {
    async function checkRoleAndLoadData() {
      try {
        ...
        if (profile.role !== "branch_report_officer") {
          setError(`Access denied. Your role is '${profile.role}', but branch_report_officer role is required.`)
          setLoading(false)
          return
        }
  ...
  if (loading) {
    return (
      <RoleLayout userRole="branch_report_officer">
  ...
  return (
    <RoleLayout userRole="branch_report_officer" userName={userProfile?.full_name}>
