...
export async function generateEmailContent(type: "invitation" | "reminder" | "report" | "notification", data: any) {
  try {
    const { text } = await generateText({
      model: openai("gpt-4o"),
...
export async function validateAndSuggestFormData(formType: string, formData: any) {
  try {
    const { text } = await generateText({
      model: openai("gpt-4o"),
...
export async function generatePredictions(historicalData: any[], predictionType: string) {
  try {
    const { text } = await generateText({
      model: openai("gpt-4o"),
